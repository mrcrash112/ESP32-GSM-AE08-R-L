...
async function loadStatus(){try{const s=await api('/api/status');if(reloadAfterReconnect){reloadAfterReconnect=false;setTimeout(()=>location.reload(),800);return}otaInProgress=updateNeedsPolling(s.update);renderUpdateButton(s.update);const paths={wifi:'WLAN',ethernet:'Ethernet',cellular:'Mobilfunk',offline:'Offline'},currentFirmware=formatVersion(s.update.currentVersion)||'–',currentRecovery=formatVersion(s.update.recoveryVersion),currentWeb=formatVersion(s.update.currentWebVersion),bridge=s.appBridge||{};$('connectionState').classList.add('online');$('connectionState').querySelector('span').textContent='Gerät verbunden';$('connectionDetail').textContent=`System MQTT: ${s.mqtt?'online':'offline'} · ${bridge.source||'The_App'}: ${bridge.online?'online':'offline'}`;$('serialNumber').textContent=s.serialNumber||'–';$('modemImei').textContent=s.modemImei||'Nicht verfügbar';$('modemType').value=s.modemModel||'Nicht erkannt';$('wifiStatus').textContent=s.wifiIp||'Nicht verbunden';$('ethernetStatus').textContent=s.ethernetIp||'Nicht verbunden';$('cellularStatus').textContent=s.cellular?'Verbunden':'Nicht verbunden';$('mqttStatus').textContent=s.mqtt?'Verbunden':'Getrennt';$('appBridgeStatus').textContent=bridge.online?`${bridge.source||'The_App'} online`:`${bridge.source||'The_App'} offline`;renderMqttConnection(s.mqttConnection||{connected:s.mqtt,code:s.mqtt?0:-1,message:s.mqtt?'Verbunden':'Getrennt',transport:''});$('firmwareVersionStatus').textContent=currentFirmware;$('dateTimeStatus').textContent=s.dateTime||'–';$('internetStatus').textContent=paths[s.internet]||s.internet||'–';$('currentFirmware').textContent=currentFirmware;$('installedDetails').textContent=`Recovery: ${currentRecovery||'Noch nicht gestartet'} · Web: ${currentWeb||'Nicht versioniert'}`;const firmwareState=$('firmwareUpdateState'),recoveryState=$('recoveryUpdateState'),webState=$('webUpdateState');firmwareState.textContent=s.update.firmwareAvailable?`Update ${formatVersion(s.update.version)} verfügbar`:'Aktuell';recoveryState.textContent=s.update.recoveryAvailable?`Update ${formatVersion(s.update.recoveryTargetVersion)} verfügbar`:'Aktuell';webState.textContent=s.update.webAvailable?`Update ${formatVersion(s.update.webTargetVersion)} verfügbar`:'Aktuell';firmwareState.className=s.update.firmwareAvailable?'update-available':'update-current';recoveryState.className=s.update.recoveryAvailable?'update-available':'update-current';webState.className=s.update.webAvailable?'update-available':'update-current';const progress=(s.update.installing||s.update.downloading||s.update.checking||s.update.downloadQueued)&&s.update.progress?` (${s.update.progress}%)`:'';const ready=s.update.available&&s.update.downloaded&&!s.update.autoInstall&&!s.update.installing&&!s.update.approved?' · Dateien geladen':'';
