...
function fillForm(c){
  ...
  setValue('webUser',c.web.user);setChecked('updateCheckEnabled',c.update.checkEnabled);setChecked('updateCellularDownloads',c.update.cellularDownloads);setChecked('updateAutoInstall',c.update.autoInstall);setValue('updateChannel',c.update.channel||'stable');setValue('updateManifestUrl',c.update.manifestUrl);setValue('updateCheckMinutes',c.update.checkMinutes);
...
function buildConfig(){const c=loadedConfig;return{
  ...
  update:{checkEnabled:checked('updateCheckEnabled'),cellularDownloads:checked('updateCellularDownloads'),autoInstall:checked('updateAutoInstall'),channel:value('updateChannel'),manifestUrl:value('updateManifestUrl'),checkMinutes:number('updateCheckMinutes')}
}}
...
async function loadStatus(){try{const s=await api('/api/status');otaInProgress=Boolean(s.update.installing||s.update.approved||s.update.downloading);const paths={wifi:'WLAN',ethernet:'Ethernet',cellular:'Mobilfunk',offline:'Offline'};$('connectionState').classList.add('online');$('connectionState').querySelector('span').textContent='Gerät verbunden';$('serialNumber').textContent=s.serialNumber||'–';$('modemImei').textContent=s.modemImei||'Nicht verfügbar';$('modemType').value=s.modemModel||'Nicht erkannt';$('wifiStatus').textContent=s.wifiIp||'Nicht verbunden';$('ethernetStatus').textContent=s.ethernetIp||'Nicht verbunden';$('cellularStatus').textContent=s.cellular?'Verbunden':'Nicht verbunden';$('mqttStatus').textContent=s.mqtt?'Verbunden':'Getrennt';renderMqttConnection(s.mqttConnection||{connected:s.mqtt,code:s.mqtt?0:-1,message:s.mqtt?'Verbunden':'Getrennt',transport:''});$('dateTimeStatus').textContent=s.dateTime||'–';$('internetStatus').textContent=paths[s.internet]||s.internet||'–';$('currentFirmware').textContent=s.update.currentVersion||'–';$('currentRecovery').textContent=s.update.recoveryVersion&&s.update.recoveryVersion!=='0.0.0'?s.update.recoveryVersion:'Noch nicht gestartet';$('currentWeb').textContent=s.update.currentWebVersion&&s.update.currentWebVersion!=='0.0.0'?s.update.currentWebVersion:'Nicht versioniert';const firmwareState=$('firmwareUpdateState'),recoveryState=$('recoveryUpdateState'),webState=$('webUpdateState');firmwareState.textContent=s.update.firmwareAvailable?`Update ${s.update.version} verfügbar`:'Hauptprogramm ist aktuell';recoveryState.textContent=s.update.recoveryAvailable?`Update ${s.update.recoveryTargetVersion} verfügbar`:'Recovery ist aktuell';webState.textContent=s.update.webAvailable?`Update ${s.update.webTargetVersion} verfügbar`:'Weboberfläche passt';firmwareState.className=s.update.firmwareAvailable?'update-available':'update-current';recoveryState.className=s.update.recoveryAvailable?'update-available':'update-current';webState.className=s.update.webAvailable?'update-available':'update-current';const progress=(s.update.installing||s.update.downloading)&&s.update.progress?` (${s.update.progress}%)`:'';const ready=s.update.downloaded&&!s.update.autoInstall?' · Dateien geladen':'';
