...
async function loadStatus(){try{const s=await api('/api/status');otaInProgress=Boolean(s.update.installing||s.update.approved);const paths={wifi:'WLAN',ethernet:'Ethernet',cellular:'Mobilfunk',offline:'Offline'};$('connectionState').classList.add('online');$('connectionState').querySelector('span').textContent='Gerät verbunden';$('serialNumber').textContent=s.serialNumber||'–';$('modemImei').textContent=s.modemImei||'Nicht verfügbar';$('modemType').value=s.modemModel||'Nicht erkannt';$('wifiStatus').textContent=s.wifiIp||'Nicht verbunden';$('ethernetStatus').textContent=s.ethernetIp||'Nicht verbunden';$('cellularStatus').textContent=s.cellular?'Verbunden':'Nicht verbunden';$('mqttStatus').textContent=s.mqtt?'Verbunden':'Getrennt';renderMqttConnection(s.mqttConnection||{connected:s.mqtt,code:s.mqtt?0:-1,message:s.mqtt?'Verbunden':'Getrennt',transport:''});$('dateTimeStatus').textContent=s.dateTime||'–';$('internetStatus').textContent=paths[s.internet]||s.internet||'–';$('currentFirmware').textContent=s.update.currentVersion||'–';$('currentRecovery').textContent=s.update.recoveryVersion&&s.update.recoveryVersion!=='0.0.0'?s.update.recoveryVersion:'Noch nicht gestartet';const firmwareState=$('firmwareUpdateState'),recoveryState=$('recoveryUpdateState');firmwareState.textContent=s.update.firmwareAvailable?`Update ${s.update.version} verfügbar`:'Hauptprogramm ist aktuell';recoveryState.textContent=s.update.recoveryAvailable?`Update ${s.update.recoveryTargetVersion} verfügbar`:'Recovery ist aktuell';firmwareState.className=s.update.firmwareAvailable?'update-available':'update-current';recoveryState.className=s.update.recoveryAvailable?'update-available':'update-current';const progress=s.update.installing&&s.update.progress?` (${s.update.progress}%)`:'';$('updateMessage').textContent=s.update.failed?`Fehler: ${s.update.message||s.update.detail||'Update fehlgeschlagen'}`:`${s.update.message||'Noch nicht geprüft'}${progress}${s.update.detail?` · ${s.update.detail}`:''}`;$('approveUpdate').hidden=!s.update.available||s.update.installing||s.update.approved}catch(error){if(otaInProgress&&networkError(error)){$('connectionState').querySelector('span').textContent='OTA läuft / Gerät startet neu';$('updateMessage').textContent='Verbindung während des Updates unterbrochen. Bitte warten …';$('approveUpdate').hidden=true;return}$('connectionState').querySelector('span').textContent='Gerät nicht erreichbar';notify(error.message,true)}}
